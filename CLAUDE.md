...
- `scenes/player.tscn` — CharacterBody3D with capsule model, walk animation, camera pivot
...
### Scripts
- `scripts/main.gd` — day/night cycle (palette blending), fog sync, terrain regeneration (R key)
- `scripts/player.gd` — WASD + sprint + jump, coyote time, apex float, procedural walk animation
...
- `shaders/water.gdshader` — vertex wave displacement, depth-buffer shore blending, metallic/specular
